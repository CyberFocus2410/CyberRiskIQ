...
def query_ai_risk_analyst(
    ...
    # Intent 1: Business Unit Risk Distribution (Specific)
    if re.search(r'\b(business unit|business units|bu|bus|division|divisions|department|departments)\b', normalized):
    ...
    # Intent 2: Budget Prioritization & Investment Optimizer
    ...
    # Intent 3: Highest Financial Cyber Risk / Highest EAL (Single Asset / Risk)
    elif (any(k in normalized for k in ["highest", "biggest", "maximum", "top", "critical"]) and any(k in normalized for k in ["risk", "eal", "exposure", "asset", "liability"])) or "highest financial" in normalized:
